...
### 2026-07-23, noob runs 5 to 5d: quick depth redesigned from inline to child-structured

The first quick design allowed inline execution when subagents were available. Three runs on fresh narrow topics (latest stable Python, Node, Rust; "use the research skill, quick:") produced correct answers from primary sources in 29 to 42 s, with two transient fetch errors self-recovered, and zero stored entries across three successive strengthenings of the storage instruction. The inline path reaches "answered" and stops on this model, every time, while deep stored every time because the child-return step structurally leads into Storage. The quick spec also requires the store write and the contrarian check; a fast path that drops them is a plain websearch, which is not what this skill's quick mode is.

The redesign removed the inline allowance: quick keeps deep's structure with a smaller brief (3 to 6 searches, one of them the contrarian check), and Storage fires on the child's return exactly as deep. Run 5d ("use the research skill, quick: whats the latest stable deno version"): 444 s, 11 calls, zero errors, child spawned with the quick brief, entry stored with `depth: quick` and dated sources, a worked `## Strongest objection` (it found and dismissed a stale third-party version listing), index row appended beside three existing entries, write verified. Quick on this stack is roughly half a deep run, and the contract holds: the depths differ in effort, never in whether the store and the contrarian happen.
