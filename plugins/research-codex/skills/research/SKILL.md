...
The data lives at `<root>/.research/`, a top-level project directory (not nested inside the agent's config directory). It is a top-level project directory chosen so research data is colocated with the project, gitignored by default, and easy to find by name. Auditing remains intact: every read and write goes through the host's normal permission system.
...
5. **Fall through to Investigation. Pick the mode:**
   ...
   - **Existing entry doesn't actually resolve the question** (problem still unsolved) → Investigation in **merge mode** (pass existing entry content to the investigation).
...
Codex subagents are optional. Use a subagent only when the user explicitly asked for subagents, delegation, or parallel agent work. If not, perform the Investigation inline using the same cognitive phases and required output format.

When subagents are authorized, spawn one research subagent with `model: "gpt-5.5"` and `reasoning_effort: "xhigh"`. The Investigation phase needs a strong model: the contrarian pass and synthesis steps depend on reasoning depth that smaller models won't deliver.

**The investigation does research and returns its synthesis as structured text. It does NOT write any files.** You (main agent) handle all file writes in Storage. This split keeps responsibility clean: the investigation starts from zero context and doesn't need to know your schema or `INDEX.md` layout.

**Naming convention.** Name or describe the task as `Research investigation: <topic>` (3 to 5 words). This makes research-skill spawns identifiable in the harness UI when the runtime exposes task names.

**On completion:** parse the return, apply Storage immediately, surface a brief notice to the user (e.g. *"research on `<topic>` saved to `<path>`"*). Do not dump the full findings into chat unless asked.

**Subagents have little or no prior context.** They may not see this skill, project instructions, or our conversation. Brief them completely and treat them as one-shot workers: fill small gaps inline, or spawn a refined follow-up only if subagents are still authorized.

The mode (new entry vs merge) and the depth (quick vs deep) were decided in Retrieval phase 5. Use them for the inline investigation or the subagent brief:

- **New entry mode** - standard brief, no existing context to feed.
- **Merge mode** - paste the existing entry's `## Summary` and any relevant `## Findings` sections into the brief, marked clearly as *"current state of the entry - verify, update, or supersede"*. Tell the investigation pass to flag claims that are now wrong.
...
- Quick keeps deep's structure: run the quick investigation as its own pass (subagent where authorized, inline otherwise) and apply Storage when its return is complete. The return is what makes Storage happen; collapsing quick into a bare lookup is how quick runs lose their write.
...
- Year-pinning rule for web searches (don't trust the model prior on what year it is)
- Effort scaling: state the expected scale in the brief - a narrow single-fact question needs ~3-10 searches; a multi-option comparison 10-15+ spread across the options; without this, investigations under- or over-invest
...
- **Merge mode only**: the existing entry content the investigation should verify / supersede
...
#### Cognitive phases (include verbatim in the subagent brief or inline notes)

The investigation walks these as discrete phases. Phases 1 and 5, and phase 6's verdict, are judgment calls; phases 2-4, phase 6's completeness pass, and the output format are followed exactly. Phases 4 and 5 are load-bearing:
...
The brief or inline investigation notes MUST include explicit citation rules. Research agents often default to `[1]`, `[2]` inline citations; without explicit instructions they will produce noisy output. State the rules in plain language. Recommended verbatim block to paste into the brief or follow inline:
...
If a subagent is used, it does not write any files. You parse this return and apply Storage rules. The `## Supersedes` section appears only in merge mode; a new-entry investigation omits it. If investigating inline, produce the same shape internally before Storage.
...
If the investigation return has gaps:

- **Small gap** (one missing fact, one specific angle) → fill it yourself with a focused web search / page fetch. Cheaper than re-spawn.
- **Large gap** (whole sections shallow, contrarian pass clearly skipped) → re-run the investigation with a refined brief that names the specific gap. The previous return is discarded (no file was written yet).
...
If the review surfaces fixable gaps, fill them yourself with a focused web search / page fetch (cheaper than re-spawn). If gaps are systemic, re-run the investigation with a refined brief; do not write a half-formed entry.

#### Apply Storage

After Investigation returns its synthesis (or the user pastes findings), you (main agent, never the subagent) finalize the data layer. This part is low-freedom: follow the steps exactly. Use Codex-safe file edits such as `apply_patch` for manual edits. Two paths, picked based on the mode chosen in Retrieval phase 5:

**New entry path:**

1. Create `<root>/.research/<topic-slug>/`.
2. Write `FINDINGS.md` using the schema in File schemas. The return maps 1:1: `## Summary`, `## Findings`, `## Insights`, `## Strongest objection` copy over; `## Discarded approaches` starts as the empty table; `## Open questions` gets any gaps flagged during review; `## Timeline` gets the initial-entry line. Include every schema section even when empty. Frontmatter: `created` and `last_verified` = today; `status: active`; `depth: deep` (or `quick` for a quick run); `sources` from the investigation return; `raw:` omitted (no raw yet); `related: []` unless cross-links apply.
...
3. **Apply the investigation's `## Supersedes` list:** move each named claim from `## Findings` to `## Discarded approaches` with date + reason. See Conflict handling.
...
3. Synthesize the content into the same shape the investigation would return (Summary / Findings / Sources). Citations to the raw file: `[Source: raw/<filename>]`.
...
- **Always run `date +%Y-%m-%d` first.** Pin the actual current year in web searches ("X 2026", "X latest 2026"). Don't trust your model's prior on what year it is.
...
- **Hallucinated sources**: never invent URLs or quotes. If fetching a source failed, say so.
