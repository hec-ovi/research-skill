...
Coverage, which counts blocked items as content-present, sits between 91% and 94% for all four arms. Every arm found the required facts. The entire spread is what each one credited them to, and that is the single most useful thing this harness has shown.

The first two skill arms scored below a plain research prompt. The judges' reasons say why: on task 80 all 11 analysis items came back -1 with wording like "explicitly attributed to Budryte's framing rather than presented independently", and on task 62 all 23 recall items were -1 because "the table is explicitly introduced as coming from the blocked Tsai et al. meta-analysis". The judge confirms the content is correct, then blocks it. The skill's own citation discipline, naming sources in prose and listing every URL, is what made its use of the origin article legible enough to penalize.

v0.3.1 attacked this by re-attributing. On task 114 that worked: best of the three arms on recall, 27 of 37 against baseline's 25 and the old brief's 19, with blocked items halved from 18 to 9, because the report traced market shares to national competition authorities instead of the blocked OECD paper. On task 62 it changed nothing, 0 of 23, because the harmonized trial-characteristics table exists only inside the blocked meta-analysis. Re-wording the credit cannot produce rows you never opened. Presentation also slipped to 47.8%, since that arm spent its effort on sourcing and let each task's requested structure drift.

v0.3.4 attacked the structure instead of the wording, and leads every dimension. Task 62 recall went from 0 of 23 to 23 of 23 with nothing blocked: the table was rebuilt row by row from the individual trials, and the judge found each study's country, design, composition, control, baseline pressures and duration present and credited to the trial. Task 114 analysis reached 8 of 8 clean and task 80 presentation recovered to 3 of 3.
...
Three tasks, one judge model, three dimensions per task. The judges apply the blocked rule at somewhat different thresholds, so a single task's reading moves the mean; the per-cell numbers above are more informative than the arm averages, and the cells are where every conclusion here came from.
